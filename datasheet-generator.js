...
import { HTMLRenderer } from './logic/html-renderer.js';
import { PDFManager } from './logic/pdf-manager.js';
import { WebResourceProvider } from './logic/webresouce-provider.js';
import fileHelper from '../lib/file-helper.cjs';

const SUBTITLE = "Product Reference Manual"
const DATASHEET_SUFFIX = "-datasheet.pdf"

const webResourceProvider = new WebResourceProvider();
 ...
 */
const generatePDFsFromMarkdownFiles = async (datasheets, targetPath) => {

    for(let datasheet of datasheets){
        webResourceProvider.addResource(datasheet);
    }

    webResourceProvider.begin();
    await pdfManager.begin();
    let tasks = [];
    
    for(let datasheet of datasheets){
        console.log(`ℹ️ Generating datasheet for ${datasheet.contentFilePath} ...`);
        tasks.push(generatePDFFromMarkdown(datasheet, targetPath).catch( error => {
            console.error(error);
            return null;
        }));
    }
...
const generatePDFFromMarkdown = async (datasheet, relativeTargetPath) => {
    ...
    htmlRenderer.addSubtitle(SUBTITLE, "SKU", identifier);
    ...
    const htmlFileURL = webResourceProvider.resourceURL(datasheet, htmlFilename)
    // console.debug("Prepare PDF \t\t--> \tstep 2 of 4")
    await pdfManager.createPDFFromURL(htmlFileURL, htmlRenderer, pdfTargetPath)
    ...
    contentListMap = await pdfManager.reverseEngineerPageNumbers(contentListMap, pdfTargetPath)	
...
export { generatePDFsFromMarkdownFiles };
